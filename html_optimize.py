...
from tp3.html import (
    HTMLTestSuiteCrossover,
    HTMLTestSuiteGenerator,
    HTMLTestSuiteMutation,
    HTMLTestSuiteProblem,
    make_executor,
)
...
def _ga_tournament(pop_size: int):
    def make(generator: HTMLTestSuiteGenerator):
        return GA(
            pop_size=pop_size,
            ...
            crossover=HTMLTestSuiteCrossover(generator=generator),
            eliminate_duplicates=False,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="TP3 - Optimize test suites for html.parser.HTMLParser().feed")
    parser.add_argument("--runs", type=int, default=5)
    parser.add_argument("--budget", type=int, default=5000)
    parser.add_argument("--pop-size", type=int, default=50)
    parser.add_argument("--seed", type=int, default=0, help="0 => random seed per run")
    parser.add_argument("--out", type=Path, default=ROOT / "results" / "htmlparser")
    parser.add_argument("--plot", type=Path, default=ROOT / "plots" / "htmlparser_convergence.png")
    ...
    ax.set_title(f"Best nl/nt over evaluations (HTMLParser.feed) — runs={args.runs}, budget={args.budget}")
    ...
    summary_path.write_text(
        json.dumps(
            {
                "sut": "html.parser.HTMLParser.feed",
